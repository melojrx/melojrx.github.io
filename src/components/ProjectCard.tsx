    ...
    <Card className="flex flex-col h-full">
      <CardHeader>
        <div className="relative w-full aspect-video overflow-hidden rounded-t-lg bg-muted">
          ...
          />
          {imageMode === "contain" && (
            <div className="pointer-events-none absolute inset-0 ring-1 ring-border/40 rounded-t-lg" />
          )}
        </div>
          ...
          <div className="space-y-3">
            {techGroups.map((group) => (
              <div key={group.label} className="flex flex-col gap-1">
                <span className="text-[10px] font-medium tracking-wide uppercase text-muted-foreground/80">{group.label}</span>
                <div className="flex flex-wrap gap-1.5">
                  {group.items.map(item => (
                    <Badge key={group.label+item} variant="outline" className="text-xs px-2 py-0.5">{item}</Badge>
                  ))}
                </div>
              </div>
            ))}
      ...
      {hasFooter && (
        <CardFooter className="flex flex-col gap-2 sm:flex-row sm:gap-3">
          {siteUrl && (
          ...
          {caseStudyUrl && (
            <Button asChild variant="ghost" size="sm" className="gap-2 flex-1 justify-start">
